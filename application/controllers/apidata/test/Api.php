...
public function newtaskcategoryvideos_post()
     ...
     function category_get(){
      $token = $_SERVER['HTTP_ACCESS_TOKEN'];
        if ($id = $this->verifyToken($token)) {
          $data = $this->common->accessrecord(TBL_CATEGORY,['CONCAT("'.BASE_URL.MUSIC_ICON.'thumb/'.'",icon) as image, name, id, parent_id'],['parent_id'=>0],'result','asc');
              foreach($data as $row){
                 
                $row->child_data=$this->common->accessrecord(TBL_CATEGORY,['CONCAT("'.BASE_URL.MUSIC_ICON.'thumb/'.'",icon) as image, name, id'],['parent_id'=>$row->id],'result','asc');
                if(!empty($row->child_data)){
                     foreach($row->child_data as $rows){
                       $rows->child_data_list = $this->common->accessrecord(TBL_CATEGORY_MUSIC,['CONCAT("'.MUSIC_VIDEO.$row->name.'/",music) as song,CONCAT("'.BASE_URL.MUSIC_ICON.$row->name."/thumb/".'",image) as thumbnail, artist,song_name,id,"00:30" as duration'],['category_id'=>$rows->id],'result');
                       foreach($rows->child_data_list as $ro){
                          $ro->is_fevorites=0;
                          if($this->common->accessrecord(TBL_USER_FEVORATES,[],['song_id'=>$ro->id,'status'=>1],'row')){
                              $ro->is_fevorites=1;
                          }
                      }
                    }
                }
                    $row->sub = $this->common->accessrecord(TBL_CATEGORY_MUSIC,['CONCAT("'.MUSIC_VIDEO.$row->name.'/",music) as song,CONCAT("'.BASE_URL.MUSIC_ICON.$row->name."/thumb/".'",image) as thumbnail, artist,song_name,id,"00:30" as duration'],['category_id'=>$row->id],'result');
          ...
          $fovorates = $this->db->query('SELECT "00:30" as duration, "1" as is_fevorites, CASE WHEN (SELECT cat.name as cate_name FROM category cat WHERE id=category.parent_id) IS NULL then  CONCAT("'.MUSIC_VIDEO.'",category.name,"/",category_music.music) else (SELECT  CONCAT("'.MUSIC_VIDEO.'",cat.name,"/",category_music.music) as cate_name FROM category cat WHERE id=category.parent_id) END as song , CASE WHEN (SELECT cat.name as cate_name FROM category cat WHERE id=category.parent_id) IS NULL then  CONCAT("'.BASE_URL.MUSIC_ICON.'",category.name,"/thumb/",category_music.image) else (SELECT  CONCAT("'.BASE_URL.MUSIC_ICON.'",cat.name,"/thumb/",category_music.image) as cate_name FROM category cat WHERE id=category.parent_id) END as thumbnail , category_music.artist,category_music.song_name,category_music.id FROM category_music inner join category on category.id = category_music.category_id inner join my_favorite on category_music.id = my_favorite.song_id WHERE '.TBL_USER_FEVORATES.'.sponsor_id='.$id['sponsor_id'].' AND '.TBL_USER_FEVORATES.'.status=1')->result();
    ...
    function UserFevorites_post(){
      ...
      if ($id = $this->verifyToken($token)) {
          ...
          if($check = $this->common->accessrecord(TBL_USER_FEVORATES,[],['song_id'=>$songid,'sponsor_id'=>$id['sponsor_id']],'row')){
              ...
              $msg = $check->status==1 ? "Song Removed From favorites list" : "Favorites Set";
          }else{
